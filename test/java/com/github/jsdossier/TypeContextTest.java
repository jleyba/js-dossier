...
public class TypeContextTest {
  ...
  public void canResolveTypesWithModuleContexts() {
    util.compile(
        createSourceFile(fs.getPath("globals.js"), "class A {}"),
        createSourceFile(fs.getPath("src/modules/foo.js"), "class A {}", "export {A as B}"),
        createSourceFile(fs.getPath("src/modules/dir/foo.js"), "class A {}", "export {A as C}"));
  ...
  public void doesNotResolveAsTypeWhenDefaultExportIsNotANominalType() {
    util.compile(
        createSourceFile(fs.getPath("src/modules/foo.js"), "export default 123;"),
  ...
  public void canResolveReexportedTypes() {
    ...
    assertThat(typeRegistry.getTypes(barB.getType())).containsExactly(fooDefault, barB).inOrder();
    assertThat(typeRegistry.getTypes(barC.getType())).containsExactly(fooA, barC).inOrder();
  }
}
